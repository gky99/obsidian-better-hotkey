...
describe('parseHotkeyString', () => {
  describe('single key', () => {
    ...
    it('parses all four modifiers: "ctrl+meta+alt+shift+x"', () => {
      ...
      expect(result).toHaveLength(1);
      expect(result[0]!.modifiers).toEqual(new Set(["ctrl", "meta", "alt", "shift"]));
      expect(result[0]!.key).toBe("x");
    });
  ...
  describe('modifier validation', () => {
    it('throws on invalid modifier: "control+x"', () => {
      expect(() => parseHotkeyString("control+x")).toThrow(/Invalid modifier "control"/);
    });

    it('throws on misspelled modifier: "crtl+x"', () => {
      expect(() => parseHotkeyString("crtl+x")).toThrow(/Invalid modifier "crtl"/);
    });
  });
  ...
  describe('edge cases', () => {
    ...
    it('throws on more than 2 chord steps', () => {
      expect(() => parseHotkeyString("ctrl+x ctrl+s ctrl+z")).toThrow(/at most 2 steps/);
    });

    it('throws on "ctrl+" (plus as base key)', () => {
      expect(() => parseHotkeyString("ctrl+")).toThrow(/Plus sign cannot be used/);
    });
