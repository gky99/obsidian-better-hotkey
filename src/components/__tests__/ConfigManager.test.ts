...
function makePresetJson(
    hotkeys: Array<{ command: string; key?: string; when?: string }>,
): string {
...
describe('ConfigManager', () => {
    ...
    describe('loadAll', () => {
        describe('preset loading', () => {
            it('loads valid preset and assigns Priority.Preset', async () => {
                ...
                expect(entries[0]!.key[0]!.modifiers).toEqual(
                    new Set(['ctrl']),
                );
        ...
        describe('user hotkey loading', () => {
            ...
            it('handles removal without key', async () => {
                files[USER_PATH] = JSON.stringify([{ command: '-kill-word' }]);
                await manager.loadAll('emacs');
            ...
            it('fires onChange with all three sources', async () => {
                ...
                files[USER_PATH] = JSON.stringify([
                    { command: 'user-cmd', key: 'ctrl+u' },
                ]);
                manager.registerPluginHotkeys('test-plugin', [
    ...
    describe('registerPluginHotkeys', () => {
        ...
        it('multiple plugins produce separate entry groups', () => {
            manager.registerPluginHotkeys('plugin-a', [
                { command: 'a:cmd', key: 'ctrl+a' },
            ]);
            manager.registerPluginHotkeys('plugin-b', [
                { command: 'b:cmd', key: 'ctrl+b' },
            ]);
    ...
    describe('addUserHotkey', () => {
        it('adds entry, persists to file, and fires onChange', async () => {
            ...
            const written = JSON.parse(files[USER_PATH]!) as Array<{
                command: string;
                key?: string;
            }>;
    ...
    describe('removeHotkey', () => {
        it('throws "not implemented" error (placeholder)', async () => {
            await expect(manager.removeHotkey(0)).rejects.toThrow(
                /Not implemented/,
            );
        });
    });
    ...
    describe('dispose', () => {
        it('clears all state and nulls onChange', async () => {
            ...
            await manager.loadAll('emacs');
            manager.registerPluginHotkeys('test', [
                { command: 'p', key: 'ctrl+p' },
            ]);
    ...
    describe('getAvailablePresets', () => {
        it('returns hardcoded list', () => {
            ...
            expect(presets).toEqual([
                { name: 'emacs', description: 'Standard Emacs keybindings' },
            ]);
        });
    });
});
