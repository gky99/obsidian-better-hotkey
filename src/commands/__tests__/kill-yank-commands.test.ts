...
describe("createKillYankCommands", () => {
	...
	describe("yank", () => {
		...
		beforeEach(() => {
			command = commands.find(
				(c) => c.id === KILL_YANK_COMMANDS.YANK,
			)!;
		});
		...
		it("inserts text from kill ring and sets yank range", async () => {
			...
			expect(context.workspaceContext.insertAtCursor).toHaveBeenCalledWith(
				"yanked",
			);
		...
		it("does nothing when kill ring is empty", async () => {
			...
			expect(context.workspaceContext.insertAtCursor).not.toHaveBeenCalled();
	...
	describe("yank-pop", () => {
		...
		it("does nothing when yankPop returns null", () => {
			...
			expect(context.workspaceContext.replaceRange).not.toHaveBeenCalled();
			expect(context.killRing.setYankRange).not.toHaveBeenCalled();
		});
